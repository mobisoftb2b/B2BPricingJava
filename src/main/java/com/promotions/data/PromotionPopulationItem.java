...
public class PromotionPopulationItem {
    ...
    public PromotionPopulationItem(String Cust_Key, String ESPNumber, int recordNumber, int recordSequence, int selectedCharacteristics, String materialCharValue, int minVarProd, int minTotQty, boolean mandatory, String UOMForMinQty, int minTotVal, Set<String> possibleItemIDs, PromotionsDataManager promotionsDataManager) {
        ...
        MinTotVal = minTotVal;
        m_CustKey = Cust_Key;
        setItems(possibleItemIDs, promotionsDataManager);
    }

    private void setItems(Set<String> possibleItemIDs, PromotionsDataManager promotionsDataManager) {
        ArrayList<String> itemCodes = promotionPopulationMapData.getItems(SelectedCharacteristics, MaterialCharValue, possibleItemIDs, promotionsDataManager);
    ...
    public boolean updateQuantity(String itemCode, float newQuantity, PromotionsDataManager promotionsDataManager) {
        ...
        if (promotionItemHeader != null) {
            isItemExist = true;
            promotionItemHeader.updateQuantity(m_CustKey, newQuantity, promotionsDataManager);
        }
        totalPCQty = 0;
        totalKarQty = 0;
        totalVal = 0;
    ...
    public void updatePriceAndDiscount(APromotionStep activePromotionStep, boolean excludeDiscount, PromotionsDataManager promotionsDataManager) {
        ...
        for (PromotionItemHeader promotionItemHeader : PromotionItemHeaderMap.getAllPromotionItemHeader()) {
            for (PromotionItem promotionItem : promotionItemHeader.getAllItem()) {
                ItemPromotionData itemPromotionData = promotionsDataManager.getOrderUIItem(promotionItemHeader.ItemCode);
...
//                    promotionItem.updateNetoTotalPrice(itemPromotionData.getItemPricingData().getTotalItemPriceNeto());
    ...
    public void resetPromotionDiscount(PromotionsDataManager promotionsDataManager) {
        for (PromotionItemHeader promotionItemHeader : PromotionItemHeaderMap.getAllPromotionItemHeader()) {
            promotionItemHeader.resetPriceAndDiscount(m_CustKey, ESPNumber, promotionsDataManager);
        }
    }

    public void resetPromotions(PromotionsDataManager promotionsDataManager) {
        for (PromotionItemHeader promotionItemHeader : PromotionItemHeaderMap.getAllPromotionItemHeader()) {
            promotionItemHeader.resetPromotions(m_CustKey, ESPNumber, promotionsDataManager);
        }
    }
    ...
    public ArrayList<ItemPromotionData> updateItemsPriceAndDiscount(String customerKey, HashMap<String, ItemPromotionData> itemsDataMap, boolean isActivePromotionStep, boolean isBonusStep, PromotionsDataManager promotionsDataManager) {
